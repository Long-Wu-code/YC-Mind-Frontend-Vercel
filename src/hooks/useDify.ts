          ...
          for await (const chunk of difyService.processStreamResponse(stream, abortControllerRef.current.signal)) {
            ...
            if (chunkContent && typeof chunkContent === 'string') {
              ...
              if (!accumulatedContentRef.current) {
                // 首次接收内容，直接显示全部内容
                console.log('First time receiving content, length:', chunkContent.length);
              ...
              } else if (chunkContent.startsWith(accumulatedContentRef.current)) {
                // 这是包含之前内容的完整消息，提取新增部分
                const newContent = chunkContent.slice(accumulatedContentRef.current.length);
                if (newContent) {
                ...
                // 可能是重置后的新内容，直接显示
                console.log('Found reset content:', JSON.stringify(chunkContent));
                accumulatedContentRef.current = chunkContent;
                ...
                // 这是完整内容，替换之前的内容
                console.log('Found complete content, replacing previous');
                accumulatedContentRef.current = chunkContent;
                fullResponse = chunkContent;
